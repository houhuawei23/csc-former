...
void IRGenerator::generate(CompileUnit *compile_unit) {
  for (auto &[_, global_variable] : compile_unit->globalVariableTable()) generate(global_variable.get());
...
void IRGenerator::generate(Instruction *inst) {
  ...
  switch (inst->itype()) {
    ...
    case PHI: {
      // for (size_t i = 0; i < static_cast<PhiInst *>(inst)->values().size(); i++)
      //   info << (i ? ", " : "") << "[ " << inst->operand(i) << ", "
      //        << inst->operand(i + static_cast<PhiInst *>(inst)->values().size()) << " ]";
      // info << "\n";
    } break;
    case CALL: {

...
std::any Visitor::visitDecl(SysYParser::DeclContext *ctx) {
  ...
  for (const auto &var_def : ctx->varDef()) {
    ...
    if (var_def->ASSIGN()) {
      ...
      if (current_zero_cnt) {
        ...
        if (!is_global) {
          if (!current_addr) {
            if (variable_addr->type()->refSize() != 1) {
  ...
  if (current_addr && dynamic_cast<IR::AddrAddInst *>(irbuilder.basicBlock()->instructions().back().get())) {
    irbuilder.basicBlock()->instructions().back()->removeAllUse();
    irbuilder.basicBlock()->instructions().back()->unUseAll();
    irbuilder.basicBlock()->instructions().pop_back();
    irbuilder.set(irbuilder.basicBlock(), irbuilder.basicBlock()->instructions().end());
  }
  return {};
}
...
std::any Visitor::visitScalarInit(SysYParser::ScalarInitContext *ctx) {
  ...
  if (c && c->type() != basic_type) {
    ...
    if (basic_type->isFloat())
      variable->initializer().emplace_back(
          new IR::InitialValue(IR::InitialValue::VALUE, IR::Constant::get((float)*c->getField<int32_t>())));
  } else {

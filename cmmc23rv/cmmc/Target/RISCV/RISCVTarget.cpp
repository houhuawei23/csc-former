...
class RISCVDataLayout final : public DataLayout {
   public:
    ...
    [[nodiscard]] size_t getPointerSize() const noexcept override { return 8; }
...
class RISCVFrameInfo final : public TargetFrameInfo {
   public:
    ...
    [[nodiscard]] bool isCallerSaved(
        const MIROperand& op) const noexcept override {
    ...
    [[nodiscard]] size_t getStackPointerAlignment(
        bool isNonLeafFunc) const noexcept override {
        CMMC_UNUSED(isNonLeafFunc);
        return 8U;
    }
    void emitPostSAPrologue(MIRBasicBlock& entryBlock,
                            const CodeGenContext& ctx,
                            int32_t stackSize) const override;
    void emitPostSAEpilogue(MIRBasicBlock& exitBlock,
                            const CodeGenContext& ctx,
                            int32_t stackSize) const override;
};
...
class RISCVRegisterInfo final : public TargetRegisterInfo {
   public:
    [[nodiscard]] bool isZeroRegister(
        const uint32_t x) const noexcept override {
        return x == RISCV::X0;
    }
    ...
    [[nodiscard]] OperandType getCanonicalizedRegisterType(
    ...
    [[nodiscard]] OperandType getCanonicalizedRegisterTypeForClass(
        uint32_t classId) const override {
        return classId == 0 ? OperandType::Int64 : OperandType::Float32;
    }
    [[nodiscard]] bool isLegalISARegOperand(const MIROperand& operand,
                                            OperandFlag flag) const override {
    ...
    [[nodiscard]] const std::vector<uint32_t>& getAllocationList(
        uint32_t classId) const override {
        if (classId == 0) {
            ...
            static std::vector<uint32_t> list{
                ...
                RISCV::X30, RISCV::X31, RISCV::X16, RISCV::X17,  //
                // $s0-$s1 for RVC
                RISCV::X8, RISCV::X9,
                // $s2-$s11
                RISCV::X18, RISCV::X19,                          //
    ...
    [[nodiscard]] MIROperand getReturnAddressRegister()
...
class RISCVTarget final : public Target {
   ...
   public:
    [[nodiscard]] const TargetOptHeuristic& getOptHeuristic()
        const noexcept override {
        static TargetOptHeuristic defaultHeuristic{
            .registerLength = 64U,
            .unrollBlockSize =
                static_cast<uint32_t>(queryTuneOpt("unroll_block_size", 4)),
            .maxUnrollBodySize = static_cast<uint32_t>(
                queryTuneOpt("max_unroll_body_size", 128)),
            .duplicationThreshold = static_cast<uint32_t>(
                queryTuneOpt("duplication_threshold", 10)),
            .duplicationIterations = static_cast<uint32_t>(
                queryTuneOpt("duplication_iterations", 10)),
            .branchLimit =
                static_cast<uint32_t>(queryTuneOpt("branch_limit", 400)),
            .disableSelectionOpt = true,
            .branchPredictionWarmupThreshold = static_cast<uint32_t>(
                queryTuneOpt("branch_prediction_warmup_threshold", 2)),
            .maxConstantHoistCount = static_cast<uint32_t>(
                queryTuneOpt("max_constant_hoist_count", 8)),
        };
    ...
    [[nodiscard]] const TargetFrameInfo& getFrameInfo()
        const noexcept override {
        return mFrameInfo;
    }
    ...
    [[nodiscard]] const TargetRegisterInfo* getRegisterInfo()
        const noexcept override {
        return &mRegisterInfo;
    }
    [[nodiscard]] const TargetScheduleModel& getScheduleModel()
        const noexcept override {
        return RISCV::getRISCVScheduleModel();
    }
    void postLegalizeFunc(MIRFunction& func,
                          CodeGenContext& ctx) const override;
    void emitAssembly(const MIRModule& module,
        ...
        auto useSysYRuntime = [&] {
            for (auto& global : module.globals()) {
                ...
                if (symbol == String::get("cmmcParallelFor") ||
                    symbol == String::get("cmmcCacheLookup") ||
                    symbol == String::get("cmmcAddRec3SRem") ||
                    symbol == String::get("cmmcReduceAddI32") ||
    ...
    void addExternalFuncIPRAInfo(MIRRelocable* symbol,
                                 IPRAUsageCache& infoIPRA) const override {
    ...
    void transformModuleBeforeCodeGen(
        Module& module,
        AnalysisPassManager& analysis) const override {
        PassManager<Module> modulePassManager;
        auto perFunc = std::make_shared<PassManager<Function>>();
        for (auto& pass : PassRegistry::get().collectFunctionPass(
                 {"CommonBaseOpt", "SCEVGEP2Phi", "DuplicateGEP",
                  "DuplicateCmp", "SDivWithPowerOf2", "DuplicateGlobal",
                  "NoSideEffectEliminate", "InstReorder"}))
...
void RISCVFrameInfo::emitPrologue(MIRFunction& mfunc,
                                  LoweringContext& ctx) const {
    const auto& args = mfunc.args();
    int32_t curOffset = 0U;
    std::vector<int32_t>
        offsets;  // off >= passingByGPR: passing by reg[off - passingByRegBase]
    ...
    for (uint32_t idx = 0; idx < args.size(); ++idx) {
        ...
        if (offset < passingByRegBase) {
            auto obj =
                mfunc.addStackObject(ctx.getCodeGenContext(), size, alignment,
                                     offset, StackObjectUsage::Argument);
            ctx.emitInst(
                MIRInst{InstLoadRegFromStack}.setOperand<0>(arg).setOperand<1>(
                    obj));
        }
    }
}

void RISCVFrameInfo::emitCall(FunctionCallInst* inst,
                              LoweringContext& ctx) const {
    auto callee = inst->lastOperand();
    auto func = dynamic_cast<Function*>(callee);
    if (!func)
        DiagnosticsContext::get()
            .attach<Reason>("dynamic call is not supported")
            .reportFatal();
    ...
    for (auto arg : inst->arguments()) {
        ...
        auto alignment =
            static_cast<int32_t>(arg->getType()->getAlignment(dataLayout));
    ...
    for (uint32_t idx = 0; idx + 1 < inst->operands().size(); ++idx) {
        ...
        const auto size =
            static_cast<uint32_t>(arg->getType()->getSize(dataLayout));
        const auto alignment = size;
        ...
        if (offset < passingByRegBase) {
            ...
            ctx.emitInst(
                MIRInst{InstStoreRegToStack}.setOperand<0>(val).setOperand<1>(
                    obj));
        }
    }
    for (uint32_t idx = 0; idx + 1 < inst->operands().size(); ++idx) {
    ...
    ctx.emitInst(MIRInst{RISCV::JAL}.setOperand<0>(
        MIROperand::asReloc(global->reloc.get())));
...
void RISCVFrameInfo::emitReturn(ReturnInst* inst, LoweringContext& ctx) const {
    if (!inst->operands().empty()) { // not empty
...
void RISCVFrameInfo::emitPostSAPrologue(MIRBasicBlock& entryBlock,
                                        const CodeGenContext& ctx,
                                        int32_t stackSize) const {
    CMMC_UNUSED(ctx);
    auto& instructions = entryBlock.instructions();
    RISCV::adjustReg(instructions, instructions.begin(), RISCV::sp, RISCV::sp,
                     -stackSize);
}
void RISCVFrameInfo::emitPostSAEpilogue(MIRBasicBlock& exitBlock,
                                        const CodeGenContext& ctx,
                                        int32_t stackSize) const {
                ...
                for (uint32_t idx = 0; idx < instInfo.getOperandNum(); ++idx) {
                    ...
                    if (operand.isReloc()) {
                        ...
                        if (operand.type() == OperandType::LowBits) {
                            const auto hiBits = MIROperand{
                                operand.getStorage(), OperandType::HighBits};
                            const auto& base = getBase();
                            if (auto it = auipcMap.find(hiBits);
                                it != auipcMap.cend()) {
            ...
            for (auto it = instructions.begin(); it != instructions.end();
                ...
                if (inst.opcode() == RISCV::AUIPC) {
                    if (it == instructions.begin() &&
                        iter != func.blocks().begin()) {
                        assert(inst.getOperand(1).type() ==
                               OperandType::HighBits);
                        auipcMap[inst.getOperand(1)][inst.getOperand(0)] =
                            getLowBits(MIROperand::asReloc(iter->get()));
                    } else {
                        auto block = makeUnique<MIRBasicBlock>(
                            String::get("pcrel").withID(
                                static_cast<int32_t>(ctx.nextId())),
                            &func);
                        auto& newInsts = block->instructions();
                        newInsts.splice(newInsts.begin(), instructions, it,
                                        instructions.end());
                    ...
                    auto& instInfo = ctx.instInfo.getInstInfo(inst);
                    for (uint32_t idx = 0; idx < instInfo.getOperandNum();
                         ++idx) {
                        auto& operand = inst.getOperand(idx);
                        if (operand.isReloc()) {
                            auto getBase = [&] {
                            ...
                            if (operand.type() == OperandType::LowBits) {
                                operand =
                                    auipcMap
                                        .at(MIROperand{operand.getStorage(),
                                                       OperandType::HighBits})
                                        .at(getBase());
                            }
                        }

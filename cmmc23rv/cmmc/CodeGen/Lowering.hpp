...
class FloatingPointConstantPool final {
    ...
    std::unordered_map<uint32_t, uint32_t>
        mFloatingPointConstant;  // represention -> index
   public:
    MIROperand getFPConstant(class LoweringContext& ctx,
...
class LoweringContext final {
    ...
    std::unordered_map<const MIRBasicBlock*,
                       std::unordered_map<ConstantFloatingPoint*, MIROperand>>
   ...
   public:
    LoweringContext(MIRModule& module,
    ...
    [[nodiscard]] OperandType getPtrType() const noexcept { return mPtrType; }

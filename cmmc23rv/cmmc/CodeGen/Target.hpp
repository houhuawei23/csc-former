...
class Target {
   public:
    ...
    [[nodiscard]] virtual const TargetOptHeuristic& getOptHeuristic()
        const noexcept;
    [[nodiscard]] virtual const DataLayout& getDataLayout() const noexcept = 0;
    [[nodiscard]] virtual const TargetScheduleModel& getScheduleModel()
        const noexcept {
    ...
    [[nodiscard]] virtual const TargetRegisterInfo* getRegisterInfo()
        const noexcept {
        reportUnreachable(CMMC_LOCATION());
    }
    virtual void transformModuleBeforeCodeGen(
        Module& module,
        AnalysisPassManager& analysis) const {
        CMMC_UNUSED(module);
        CMMC_UNUSED(analysis);
    }
    virtual void transformModuleBeforeOpt(Module& module,
                                          AnalysisPassManager& analysis) const {
        CMMC_UNUSED(module);
        CMMC_UNUSED(analysis);
    }
    [[nodiscard]] virtual bool isNativeSupported(
        InstructionID inst) const noexcept {
        CMMC_UNUSED(inst);
        return true;
    }
    virtual void postLegalizeFunc(MIRFunction& func,
                                  CodeGenContext& ctx) const {
        CMMC_UNUSED(func);
        CMMC_UNUSED(ctx);
    }
    virtual void emitAssembly(const MIRModule& module,
                              std::ostream& out,
    ...
    virtual void addExternalFuncIPRAInfo(MIRRelocable* symbol,
                                         IPRAUsageCache& infoIPRA) const {
        CMMC_UNUSED(symbol);
        CMMC_UNUSED(infoIPRA);
    }
    [[nodiscard]] virtual bool isLibCallSupported(
        CMMCLibCall libCall) const noexcept {
        CMMC_UNUSED(libCall);
        return false;
    }
};
...
struct CodeGenContext final {
    ...
    MIRFlags flags;
    uint32_t idx = 0;
    uint32_t nextId() { return ++idx; }
};

using TargetBuilder =
    std::pair<std::string_view, std::function<std::unique_ptr<Target>()>>;

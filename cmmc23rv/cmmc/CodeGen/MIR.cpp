...
bool MIRBasicBlock::verify(std::ostream& out, const CodeGenContext& ctx) const {
    if (ctx.flags.endsWithTerminator) {
        if (mInsts.empty())
            return false;
        if (!requireFlag(ctx.instInfo.getInstInfo(mInsts.back()).getInstFlag(),
                         InstFlagTerminator))
            return false;
        for (auto& inst : mInsts) {
            if (&inst != &mInsts.back() &&
                requireFlag(ctx.instInfo.getInstInfo(inst).getInstFlag(),
                            InstFlagTerminator)) {
                return false;
            }
        }
...
void MIRFunction::dump(std::ostream& out, const CodeGenContext& ctx) const {
    ...
    for (auto& [ref, obj] : mStackObjects) {
        out << "  so" << (ref.reg() ^ stackObjectBegin)
    ...
    for (const auto& val : mData)
        std::visit(
            Overload{
                ...
                [&](const std::byte byte) {
                    out << "\t.byte\t" << static_cast<uint32_t>(byte) << '\n';
                }},
            val);
}
...
MIROperand MIRFunction::addStackObject(CodeGenContext& ctx,
                                       uint32_t size,
                                       uint32_t alignment,
                                       int32_t offset,
